...
def build_mcp_tool_interceptors(
    ...
    *,
    ...
    """Build OAuth, user auth, context headers, then custom MCP interceptors."""
    ...
    # Context headers are the most specific built-in source: they are supplied
    # by the current request, so register them closer to the transport than
    # static OAuth or user-id mappings. A custom interceptor remains able to
    # make an explicit final override because it is registered afterwards.
