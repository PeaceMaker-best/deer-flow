...
Multi-tenant HTTP/SSE servers can map request-scoped `config.context.secrets`
to per-call headers with `headers_from_context`; missing values fail closed and
are never persisted with the run.
